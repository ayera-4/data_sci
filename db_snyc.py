...
class MongoRepository:
    ...
    def get_nationality_value_counts(self, normalize=True):
        ...
        df = pd.DataFrame(result).rename({"_id": "country_iso2"}, axis="columns").sort_values("count")
        
        # Add country names and ISO3
        cc = CountryConverter()
        df["country_name"] = cc.convert(
            df["country_iso2"], to="name_short"
        )
        df["country_iso3"] = cc.convert(
            df["country_iso2"], to="ISO3"
        )
        
        # Transform frequency count to pct
        if normalize:
            df["count_pct"] = (df["count"] / df["count"].sum()) *100
#         
        # Return DataFrame
        return df
